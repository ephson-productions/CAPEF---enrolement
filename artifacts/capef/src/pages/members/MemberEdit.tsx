...
export default function MemberEdit() {
  ...
  const onSubmit = async (data: MemberFormValues) => {
    ...
    try {
      ...
      toast({ title: 'Succès', description: 'Enrôlement mis à jour avec succès.' });
      setLocation(`/members/${id}`);
    } catch (err) {
      console.error(err);
      toast({ variant: 'destructive', title: 'Erreur', description: 'Impossible de mettre à jour cet enrôlement.' });
    }
  };

  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground animate-pulse">Chargement du formulaire...</div>;
  }
  ...
  return (
    <div>
      <div className="mb-8 max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold text-foreground">Modifier l'Enrôlement</h1>
        <p className="text-muted-foreground mt-1">{member.memberNumber}</p>
      </div>
      <MemberForm member={member} isSubmitting={updateMember.isPending} onSubmit={onSubmit} submitLabel="Enregistrer les modifications" />
    </div>
  );
}
