...
const statusLabels: Record<AppUserStatus, string> = { active: 'Actif', suspended: 'Suspendu', banned: 'Banni' };
...
export default function UserDetail() {
  ...
  const addZone = () => {
    if (!selectedReg) {
      toast({ variant: 'destructive', title: 'Zone incomplète', description: 'Sélectionnez au moins une région.' });
      return;
    }
    ...
    if (assignedZones.some((item) => item.regionId === zone.regionId && (item.departmentId || null) === (zone.departmentId || null) && (item.arrondissementId || null) === (zone.arrondissementId || null))) {
      toast({ variant: 'destructive', title: 'Zone déjà ajoutée', description: 'Cette combinaison existe déjà.' });
      return;
    }
  ...
  const upload = async (event: React.ChangeEvent<HTMLInputElement>, field: 'cni' | 'profile') => {
    ...
    try {
      ...
      toast({ title: 'Photo téléversée', description: 'La photo sera enregistrée avec la fiche.' });
    } catch {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (isSelf && (role !== user.role || JSON.stringify(assignedZones) !== JSON.stringify(user.assignedZones || []))) {
      toast({ variant: 'destructive', title: 'Modification refusée', description: 'Vous ne pouvez pas modifier votre propre rôle ou vos zones.' });
      return;
    }
    if (status !== user.status && status !== 'active' && !window.confirm(`Confirmer la ${status === 'banned' ? 'désactivation définitive' : 'suspension'} de ${user.name} ?`)) return;
    try {
      ...
      await refetch();
      toast({ title: 'Fiche mise à jour', description: 'Les informations de l’utilisateur ont été enregistrées.' });
    } catch (error: any) {
      toast({ variant: 'destructive', title: 'Erreur', description: error?.response?.data?.error || 'Impossible de mettre à jour la fiche.' });
    }
  };

  const remove = async () => {
    if (!user || !window.confirm(`Supprimer définitivement le compte de ${user.name} ? Cette action supprimera aussi son identité Clerk.`)) return;
    try {
      await deleteUser.mutateAsync({ id: user.id });
      toast({ title: 'Utilisateur supprimé', description: 'Le compte ne pourra pas être recréé automatiquement.' });
      setLocation('/users');
    } catch (error: any) {
      toast({ variant: 'destructive', title: 'Suppression impossible', description: error?.response?.data?.error || 'Le compte n’a pas été supprimé.' });
    }
  };
  ...
  if (!user) return <div className="space-y-4"><Link href="/users" className="inline-flex items-center text-muted-foreground"><ArrowLeft className="mr-2 h-4 w-4" /> Retour</Link><p className="text-muted-foreground">Utilisateur introuvable.</p></div>;

  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-12">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/users" className="inline-flex items-center font-medium text-muted-foreground transition-colors hover:text-foreground"><ArrowLeft className="mr-2 h-4 w-4" /> Retour à la liste</Link>
        <button type="button" onClick={remove} disabled={deleteUser.isPending} className="inline-flex items-center gap-2 rounded-md border border-destructive/30 px-3 py-2 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><Trash2 className="h-4 w-4" /> Supprimer</button>
      </div>

      <div>
        <h1 className="text-2xl font-bold text-foreground">Fiche utilisateur</h1>
        <p className="mt-1 text-sm text-muted-foreground">Les changements d’accès sont réservés aux administrateurs.</p>
      </div>
      ...
      <form onSubmit={save} className="space-y-6 rounded-xl border border-border bg-card p-6 shadow-sm">
        <div className="flex flex-col gap-5 border-b border-border pb-6 sm:flex-row sm:items-center">
          ...
          <div><h2 className="text-xl font-bold">{user.name}</h2><p className="text-sm text-muted-foreground">{user.email}</p><p className="mt-1 text-xs text-muted-foreground">Identifiant Clerk : {user.clerkUserId}</p></div>
        </div>

        <div className="grid gap-5 md:grid-cols-2">
          <label className="text-sm font-medium">Nom complet<input value={name} onChange={(event) => setName(event.target.value)} className="mt-1 w-full rounded-md border border-input bg-background p-2.5 text-sm" required /></label>
          <label className="text-sm font-medium">Email <span className="font-normal text-muted-foreground">(synchronisé depuis Clerk)</span><input value={user.email} readOnly className="mt-1 w-full cursor-not-allowed rounded-md border border-input bg-muted p-2.5 text-sm text-muted-foreground" /></label>
          <label className="text-sm font-medium">Rôle
            <select value={role} disabled={isSelf} onChange={(event) => setRole(event.target.value as AppUserUpdateRole)} className="mt-1 w-full rounded-md border border-input bg-background p-2.5 text-sm disabled:cursor-not-allowed disabled:bg-muted">
              <option value="agent">Agent de terrain</option><option value="supervisor">Superviseur régional</option><option value="admin">Administrateur</option>
            </select>
          </label>
          ...
          {isSelf && <p className="text-xs text-muted-foreground md:col-span-2">Votre propre rôle et vos zones sont verrouillés. Utilisez « Mon profil » pour vos documents et photos.</p>}
        </div>

        <div className="grid gap-5 border-t border-border pt-5 md:grid-cols-2">
          <label className="text-sm font-medium">Numéro de CNI<input value={cniNumber} onChange={(event) => setCniNumber(event.target.value)} className="mt-1 w-full rounded-md border border-input bg-background p-2.5 text-sm" placeholder="Numéro d’identité" /></label>
          <div className="text-sm font-medium">Photo de CNI
        ...
        <div className="space-y-4 border-t border-border pt-5">
          <div><h3 className="font-bold">Zones assignées</h3><p className="text-xs text-muted-foreground">Ajoutez plusieurs combinaisons région, département et arrondissement.</p></div>
          <div className="grid gap-3 sm:grid-cols-4">
            <select disabled={isSelf} value={selectedReg || ''} onChange={(event) => { setSelectedReg(event.target.value ? Number(event.target.value) : null); setSelectedDept(null); setSelectedArr(null); }} className="rounded-md border border-input bg-background p-2 text-xs disabled:bg-muted"><option value="">Région</option>{regions?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
            <select disabled={isSelf || !selectedReg} value={selectedDept || ''} onChange={(event) => { setSelectedDept(event.target.value ? Number(event.target.value) : null); setSelectedArr(null); }} className="rounded-md border border-input bg-background p-2 text-xs disabled:bg-muted"><option value="">Tous les départements</option>{departments?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
            <select disabled={isSelf || !selectedDept} value={selectedArr || ''} onChange={(event) => setSelectedArr(event.target.value ? Number(event.target.value) : null)} className="rounded-md border border-input bg-background p-2 text-xs disabled:bg-muted"><option value="">Tous les arrondissements</option>{arrondissements?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
            <button type="button" disabled={isSelf} onClick={addZone} className="inline-flex items-center justify-center gap-1 rounded-md bg-secondary px-3 py-2 text-xs font-semibold text-secondary-foreground hover:bg-secondary/90 disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-4 w-4" /> Ajouter</button>
          </div>
          <div className="space-y-2">
            {!assignedZones.length && <p className="rounded-md border border-dashed p-4 text-center text-xs text-muted-foreground">Aucune zone spécifique assignée.</p>}
            {assignedZones.map((zone, index) => <div key={`${zone.regionId}-${zone.departmentId}-${zone.arrondissementId}-${index}`} className="flex items-center justify-between gap-3 rounded-md border bg-muted/10 px-3 py-2 text-xs"><span><MapPin className="mr-1 inline h-3.5 w-3.5 text-primary" />{regionNames.get(zone.regionId) || `Région #${zone.regionId}`} {zone.departmentId ? `· Département #${zone.departmentId}` : '· Tous les départements'} {zone.arrondissementId ? `· Arrondissement #${zone.arrondissementId}` : ''}</span><button type="button" disabled={isSelf} onClick={() => setAssignedZones((items) => items.filter((_, itemIndex) => itemIndex !== index))} className="text-destructive disabled:opacity-50"><X className="h-4 w-4" /></button></div>)}
          </div>
        </div>

        <div className="flex justify-end border-t border-border pt-5"><button type="submit" disabled={updateUser.isPending || uploadFile.isPending} className="inline-flex items-center gap-2 rounded-md bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50"><Save className="h-4 w-4" /> {updateUser.isPending ? 'Enregistrement…' : 'Enregistrer les modifications'}</button></div>
      </form>
    </div>
  );
}